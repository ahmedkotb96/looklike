...
// Defines props for the UI component, including those from useSortable and standard HTML attributes
...
const EditModal = ({ member, isOpen, onClose, onSave, loading }: { member: TeamMember | null; isOpen: boolean; onClose: () => void; onSave: (member: TeamMember) => void; loading: boolean }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40 animate-fade-in">
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-800">{formData.id ? 'Edit' : 'Add'} Team Member</h3>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100"><X size={20} /></button>
                </div>
                <div className="space-y-4">
                    <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Full Name" className="w-full px-4 py-2 border border-slate-300 rounded-lg" />
                    <input type="text" name="role" value={formData.role} onChange={handleChange} placeholder="Role / Position" className="w-full px-4 py-2 border border-slate-300 rounded-lg" />
...
const SortableItemUI = React.forwardRef<HTMLLIElement, SortableItemUIProps>(
    ({ member, onEdit, onRemove, listeners, ...props }, ref) => (
        <li ref={ref} {...props} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="flex items-center gap-2">
                <button onClick={() => onEdit(member)} className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Pencil size={18} /></button>
                <button onClick={() => onRemove(member.id)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors"><Trash2 size={18} /></button>
            </div>
        </li>
    )
);
...
const SortableItem = ({ member, onEdit, onRemove }: { member: TeamMember, onEdit: (member: TeamMember) => void, onRemove: (id: string) => void}) => {
    const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: member.id });
    const style = { transform: CSS.Transform.toString(transform), transition };
    return <SortableItemUI ref={setNodeRef} style={style} listeners={listeners} {...attributes} member={member} onEdit={onEdit} onRemove={onRemove} />;
};
...
const AdminTeam = () => {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setTeamMembers((items) => arrayMove(items, items.findIndex(i => i.id === active.id), items.findIndex(i => i.id === over.id)));
            setIsDirty(true);
        }
    };
    ...
    const handleSaveOrder = async () => {
        ...
        teamMembers.forEach((member, index) => {
            batch.update(doc(db, 'teamMembers', member.id), { order: index });
        });
        try {
            await batch.commit();
    ...
    const handleOpenModal = (member: TeamMember | null = null) => {
        setEditingMember(member || { id: '', name: '', role: '', image: '', order: teamMembers.length });
        setIsModalOpen(true);
    };

    const handleSaveMember = async (memberToSave: TeamMember) => {
        setLoading(true);
        try {
            if (memberToSave.id) {
                await updateDoc(doc(db, 'teamMembers', memberToSave.id), { ...memberToSave });
                showNotification('Member updated successfully!', 'success');
            } else {
                await addDoc(collection(db, 'teamMembers'), memberToSave);
    ...
    return (
        <div className="animate-fade-in">
            ...
            <EditModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} member={editingMember} onSave={handleSaveMember} loading={loading} />
            ...
            {loading && teamMembers.length === 0 ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={teamMembers.map(m => m.id)} strategy={verticalListSortingStrategy}>
                        <ul className="space-y-3">
                            {teamMembers.map(member => <SortableItem key={member.id} member={member} onRemove={handleRemoveMember} onEdit={handleOpenModal} />)}
                        </ul>
                    </SortableContext>
                </DndContext>
            ) : (
                <div className="text-center py-16 bg-white rounded-xl shadow-md border border-slate-200">
                    ...
                    <h3 className="mt-2 text-xl font-semibold text-slate-700">No Team Members Found</h3>
                    <p className="mt-1 text-sm text-slate-500">Click "Add Member" to get started.</p>
                </div>
            )}
        </div>
    );
